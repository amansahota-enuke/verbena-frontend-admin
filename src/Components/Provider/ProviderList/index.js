...
function ProviderList() {
  ...
  const updateProviderStatus = async (providerId, value) => {
    try {
      const response = await ProviderService.updateProviderStatus(providerId, {
        status: value ? 1 : 0,
      });
  ...
  const openSignupConfirmation = () => {
    dispatch(
      ConfirmationActions.setConfirmationType(confirmationConstants.SIGNUP_LINK)
    );
                  ...
                  providerList.map((provider) => (
                    <tr key={provider.id}>
                      <td className="px-6 dark-gray-color py-4 whitespace-nowrap text-center font-18">
                        {`${parseName(provider.first_name)} ${parseName(
                          provider.last_name
                        )}`}
                      </td>
                      <td className="px-6 dark-gray-color py-4 whitespace-nowrap text-center font-18">
                        {provider.provider_speciality_master.name}
                      </td>
                      <td className="px-6 dark-gray-color py-4 whitespace-nowrap text-center font-18">
                      ...
                      <td className="px-6 dark-gray-color py-4 whitespace-nowrap text-center font-18">
                        {moment(provider.created_on).format("MM-DD-YYYY")}
                      </td>
                      <td className="px-6 dark-gray-color py-4 whitespace-nowrap text-center font-18">
                        <Switch
                          checked={Number(provider.status) === 1 ? true : false}
                          onChange={(e) => updateProviderStatus(provider.id, e)}
                          className={`${
